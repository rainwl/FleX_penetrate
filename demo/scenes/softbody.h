...
public:
  virtual void AddInstance(Instance instance) { this->mInstances.push_back(instance); }
    ...
    for (int x = 0; x < xStack; ++x) {
      for (int y = 0; y < yStack; ++y) {
        for (int z = 0; z < zStack; ++z) {
          ...
          if (rotateColors) { instance.mColor = mColorPicker[(x * yStack * zStack + y * zStack + z) % 7]; }
  ...
  virtual void Initialize() {
    ...
    // build soft bodies 
    for (int i = 0; i < int(mInstances.size()); i++) CreateSoftBody(mInstances[i], mRenderingInstances.size());

    g_buffers->positions[g_buffers->positions.size()].w = 0;
    // fix any particles below the ground plane in place
    for (int i = 0; i < int(g_buffers->positions.size()); ++i) if (g_buffers->positions[i].y < 0.0f) g_buffers->positions[i].w = 0.0f;
  ...
  void CreateSoftBody(Instance instance, int group = 0) {
    ...
    for (int i = 0; i < asset->numShapes; ++i) renderingInstance.mRigidRestPoses.push_back(Vec3(&asset->shapeCenters[i * 3]));
    ...
    for (int i = 0; i < asset->numShapeIndices; ++i) g_buffers->rigidIndices.push_back(asset->shapeIndices[i] + particleOffset);
...
public:
  ...
  virtual void Initialize() {
    ...
    for (int i = 0; i < int(g_buffers->positions.size()); ++i) if (g_buffers->positions[i].x < mRadius) g_buffers->positions[i].w = 0.0f;
  }
